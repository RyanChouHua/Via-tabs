...
4. 强制停止 Via 并重新打开。
5. 在 Via 页面右侧看到 `标签` 悬浮按钮。
6. 点击 `标签`，确认可以打开中文操作面板。
7. 使用面板执行 `保存当前标签到书签`，确认 Via 书签中出现对应文件夹。
8. 使用面板执行 `创建标签分组`，确认 `tab-groups.json` 追加分组。
9. 使用面板执行 `恢复最近分组`，确认可恢复最近未归档分组。
10. 使用面板执行 `归档最近分组` 和 `删除最近分组`，确认日志无异常。
